...
int check_period(char *string)
{
  ...
  if (string[0] == '.') // Đầu tiên không thể là dấu chấm
    return 0;
  if (string[n - 1] == '.') // Ký tự cuối cùng không thể là dấu chấm
    return 0;
  for (int i = 0; i < n - 1; i++)
  {
    if (string[i] == '.') 
...
/* Kiểm tra xem input có là địa chỉ IP không
Output: 1 - là địa chỉ IP, 0 - không là địa chỉ IP */
int check_IP(char *string)
{
  int value = 0, n = strlen(string);

  // điều kiện dấu chấm không thỏa mãn thì không phải là địa chỉ IP 
  ...
  else
  {
    for (int i = 0; i < n; i++)
    {
      if (string[i] == '.')
      {
        if (value < 0 || value > 255) // giá trị địa chỉ IP của 8 bit nằm trong khoảng 0 - 255
          return 0;
        value = 0;
      }
      else
