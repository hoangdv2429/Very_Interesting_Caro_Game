...
// Very Interesting Caro Game - Server
#define STATUS_START_MENU 0
#define STATUS_VERYINTERESTINGCAROGAME_MENU 1
#define STATUS_HANDLE_GAME 2
#define SIGNAL_CHECKLOGIN "SIGNAL_CHECKLOGIN"
#define SIGNAL_REGISTER "SIGNAL_REGISTER"
#define SIGNAL_OK "SIGNAL_OK"
#define SIGNAL_ERROR "SIGNAL_ERROR"
#define SIGNAL_CLOSE "SIGNAL_CLOSE"

// Caro game
// not used yet, will be used to play with players
#define SIGNAL_CARO_NEWGAME_WITH_PLAYERS "SIGNAL_CARO_NEWGAME_WITH_PLAYERS"
// used to play with CPU
#define SIGNAL_CARO_NEWGAME "SIGNAL_CARO_NEWGAME"
#define SIGNAL_CARO_ABORTGAME "SIGNAL_CARO_ABORTGAME"
#define SIGNAL_CARO_PC_TURN "SIGNAL_CARO_PC_TURN"
...
#define CARO_INFO_WIN "You Win. Do you want to view game's log (y or n)?"
...
// client variable
int status; // status of VeryInterestingCaroGame
char choice, token[] = "#";
...
// trang thai choi caro
/*
1 là chơi với CPU
2 là chơi với người chơi khác
3 là xem ranking
4 là quit
*/ 
int checkSubMenuOption;
...
int menuSignin()
{
  ...
  {
    clearScreen();
    printf("----------VeryInterestingCaroGame----------\n");
    printf("\tSign in\n");
    printf("---------------------------\n");
    if (error[0] != '\0')
    {
...
int menuRegister()
{
  ...
  while (1)
  {
    ...
    if (strcmp(pass, comfirmPass) == 0)
    {
      ...
      sprintf(send_msg, "%s#%s#%s", SIGNAL_REGISTER, user, pass);
      if (connectToServer() == 0)
      {
        str = strtok(recv_msg, token);
        if (strcmp(str, SIGNAL_OK) == 0)
...
int menuStart()
{
  ...
  while (1)
  {
    ...
    else if (choice == '2')
    {
      ...
      sprintf(error, "No such option %c", choice);
  }
...
int menuCaroGame()
{
  ...
  while (1)
  {
    ...
    {
      sprintf(error, "Size %d is out of range, size error!", size);
    }
  }
  return 0;
}
...
int handleCaroRanking()
{
  ...
  if (connectToServer() == 0)
  {
    ...
    if (strcmp(str, SIGNAL_OK) == 0)
    {
      ...
      if (numberOfWin == -1)
      {
        printf("You haven't played any matchs yet . You are not in the ranking.\n");
      }
      else
      {
        str = strtok(NULL, token);
...
/* Hiển thị phần xử lí game, chọn game caro với máy hoặc với người */
int menuVeryInterestingCaroGame()
{
  error[0] = '\0';
  while (1)
  {
    ...
    printf("\t1.Caro game\n");
    printf("\t2.Caro game with online players! (Under development)\n");
    printf("\t2.Caro ranking\n");
    printf("\t4.Sign out\n");
    printf("---------------------------\n");
    printf("Your choice: ");
    scanf("%c", &choice);
    ...
    if(choice == '2')
    {
      sprintf(error, "Under Development :(\n");
    }
    else if (choice == '3')
    {
      checkSubMenuOption = 3;
      break;
    }
    else if (choice == '4')
    {
    ...
    {
      sprintf(error, "%c is not an option", choice);
    }
  }
  return 0;
}
...
int handleCaroGame()
{
  ...
  while (1)
  {
    ...
    {
      sprintf(send_msg, "%s#%s#%s#%d#%d", SIGNAL_CARO_PC_TURN, id, user, col, row);
      if (connectToServer() == 0)
      {
        str = strtok(recv_msg, token);
        if (strcmp(str, SIGNAL_CARO_PC_TURN) == 0)
        {
          str = strtok(NULL, token);
          col = atoi(str);
          str = strtok(NULL, token);
  ...
  while (1)
  {
    if (status == STATUS_START_MENU)
    {
      if (menuStart() == -1)
        ...
        status = STATUS_VERYINTERESTINGCAROGAME_MENU;
    }
    else if (status == STATUS_VERYINTERESTINGCAROGAME_MENU)
    {
      if (menuVeryInterestingCaroGame() == -1)
        status = STATUS_START_MENU;
      else
        status = STATUS_HANDLE_GAME;
    }
    else if (status == STATUS_HANDLE_GAME) //new game with PC
    {
      if (checkSubMenuOption == 1) 
      {
        if (handleCaroGame() == -1)
        {
          free(table);
      ...
      else if (checkSubMenuOption == 3) // check ranking 
      {
        handleCaroRanking();
        status = STATUS_VERYINTERESTINGCAROGAME_MENU;
      }
