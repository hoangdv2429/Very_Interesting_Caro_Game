...
#define SIGNAL_CARO_VIEWLOG_PVP "SIGNAL_CARO_VIEWLOG_PVP"
// used
...
void updateCaroRanking( char* user, int winOrLost); /* update caro ranking */
int handleDataFromClient(int fd); /* Xử lí dữ liệu gửi từ client */
...
int handleDataFromClient(int fd){
  ...
  else if(strcmp(str, SIGNAL_REGISTER) == 0){
    // Create new user
  ...
  else if(strcmp(str, SIGNAL_CARO_PC_TURN) == 0){
    ...
    if(info != NULL){
      ...
      if(playerMove(col, row) == 1){
        ...
        updateCaroRanking(user, 1); // update caro Ranking, win = 1
      	...
      	sprintf(send_msg, "%s#%d#%d", SIGNAL_CARO_PC_TURN, col, row);
      	send(fd, send_msg, strlen(send_msg), 0);
      }
      else{
      	//write log
...
int main(int argc, char *argv[]){
  ...
  printf("VICG waiting for client on port %d\n", PORT);
  fflush(stdout);
